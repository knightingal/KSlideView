...
public class YImageView extends ImageView {
    private static final String TAG = "YImageView";
    public final int id;

    private YImageSlider yImageSlider;

    public void setLocationIndex(int locationIndex) {
        Log.d(TAG, "set locationIndex " + locationIndex + " to yImageView " + id);
    ...
    public YImageView(Context context, YImageSlider yImageSlider, int locationIndex, int id) {
        super(context);
        this.id = id;
    ...
    protected boolean setFrame(int l, int t, int r, int b) {
        ...
        if (bitmap_W < screamW) {
            float rat = ((float)bitmap_H)/((float)bitmap_W);
            bitmap_W = screamW;
            bitmap_H = (int)(rat * bitmap_W);
        }
        ...
        originY = top;
        bottom = top + bitmap_H;
        Log.d(TAG, "setFrame to " + id + " left:" + left + " top:" + top + " right:" + right + " bottom:" + bottom);
        boolean isChanged = super.setFrame(0, top, bitmap_W, top + bitmap_H);
        setX(left);
//        setY(top);
        return isChanged;
    }
    ...
    private void hideImageAnimBack() {
        AnimatorSet setYE = new AnimatorSet();
        setYE.playTogether(
                ObjectAnimator.ofFloat(yImageSlider.getHideRight(), View.Y, yImageSlider.getHideRight().getY(), yImageSlider.getHideRight().originY),
                ObjectAnimator.ofFloat(yImageSlider.getHideLeft(), View.Y, yImageSlider.getHideLeft().getY(), yImageSlider.getHideLeft().originY)
        );
        setYE.setDuration(ANIM_DURATION);
        setYE.setInterpolator(new AccelerateInterpolator());
        setYE.start();
    ...
    public void doBackImgAnim() {
        ...
        animators.add(ObjectAnimator.ofFloat(
                ...
                View.X,
                this.getX(),
                yImageSlider.getHideLeft().getBitmap_W() + YImageSlider.SPLITE_W)
        );
        animators.add(ObjectAnimator.ofFloat(
                this,
                View.Y,
        ...
        if (yImageSlider.getHideLeft().isDisplay) {
            animators.add(ObjectAnimator.ofFloat(
                    yImageSlider.getHideLeft(),
                    View.X,
                    yImageSlider.getHideLeft().getX(),
                    0
            ));
            animators.add(ObjectAnimator.ofFloat(
                    yImageSlider.getHideLeft(),
                    View.Y,
                    yImageSlider.getHideLeft().getY(),
                    yImageSlider.getHideLeft().originY
            ));
        }
    ...
    public void doNextImgAnim() {
        ...
        animators.add(ObjectAnimator.ofFloat(this, View.Y, this.getY(), originY));
        if (yImageSlider.getHideRight().isDisplay) {
            animators.add(ObjectAnimator.ofFloat(
                    yImageSlider.getHideRight(),
                    View.X,
                    yImageSlider.getHideRight().getX(),
                    -(yImageSlider.getHideRight().getBitmap_W() - screamW)
            ));
            animators.add(ObjectAnimator.ofFloat(
                    yImageSlider.getHideRight(),
                    View.Y,
                    yImageSlider.getHideRight().getY(),
                    yImageSlider.getHideRight().originY
            ));
        }
        setX.playTogether(animators);
        setX.setDuration(ANIM_DURATION);
...
//        setY.play(ObjectAnimator.ofFloat(this, View.Y, this.getY(), animDataY.dest));
    ...
    void onTouchMove(MotionEvent event) {
        ...
        yImageSlider.getHideLeft().addDiff(diffX, 0);
        yImageSlider.getHideRight().addDiff(diffX, 0);
